...
class Oil(Point):
    weights = pylab.array([1.0] * 73)  ## This is a class variable, not instance variable
    ...
    def distance(self, other):
        ...
        return sum(Oil.weights * difference * difference) ** 0.5
...
class Cluster(object):
    ...
    def computeCentroid(self):
        ...
        for p in self.points:
            totVals += p.getAttrs()
        meanPoint = self.pointType('mean',totVals/float(len(self.points)),totVals/float(len(self.points)))
        return meanPoint
...
#US Counties example
def readOilData(fName, numEntries = 73):
...
def buildOilPoints(fName, scaling = True):
    """
    Given an input filename, reads County values from the file and returns
    them all in a list.
    """
    nameList, featureList, maxVals = readOilData(fName)
    points = []
    for i in range(len(nameList)):
        ...
        #normalizedAttrs = originalAttrs/pylab.array(maxVals)
        #print pylab.array(maxVals)
        points.append(Oil(nameList[i], originalAttrs, normalizedAttrs))
    return points

def randomPartition(l, p): #l = list, not 1
    """
...
#    incomes = []
#    print ''
#    clusters, maxSmallest = kmeans(points, k, cutoff, Oil)
#
#    for i in range(len(clusters)):
...
#points = buildOilPoints('counties.txt')
#random.seed(123)
#testPoints = random.sample(points, len(points)/10)
#test(points, k = 200, cutoff = 0.1)

def testShowHistory(k, cutoff = 0.0001, numTrials = 10, printSteps = False,
          printHistory = False):
    points = buildOilPoints('OilEconomyFRED.txt')
    if printSteps:
        print 'Points:'
        for p in points:
    ...
    while numClusterings < numTrials:
        clusters, maxDiameter = kmeans(points, k, cutoff, Oil)
...
testShowHistory(k = 3, cutoff = 0.0001, numTrials = 10, printSteps = False, printHistory = False)
